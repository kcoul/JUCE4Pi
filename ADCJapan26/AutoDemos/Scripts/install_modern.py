"""
install_modern.py
Writes Modern.fxp to the SurgeXT user Patches folder.
No OSC or running SurgeXT required — pure filesystem write.

Local (run on target):
    python install_modern.py --user-patch-dir "/home/user/Documents/Surge XT/Patches"

Remote (run on host, copies via SCP):
    python install_modern.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip 192.168.1.100
    python install_modern.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip user@192.168.1.100
...
_XML = (
    ...
    '<patch revision="9">'
    '<meta name="Modern" category="QNX" comment="" author="QNX" />'
    '<parameters>'
...
def _deploy_local(fxp_bytes: bytes, user_patch_dir: str) -> None:
    ...
    out_path = os.path.join(out_dir, "Modern.fxp")
...
def _deploy_remote(fxp_bytes: bytes, user_patch_dir: str, target: str) -> None:
    ...
    remote_path = remote_dir + "/Modern.fxp"
...
def main():
    parser = argparse.ArgumentParser(description="Install Modern.fxp to SurgeXT User Patches > QNX")
