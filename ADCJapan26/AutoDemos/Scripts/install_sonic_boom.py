"""
install_sonic_boom.py
Writes SonicBoom.fxp to patches_3rdparty/QNX/ in the SurgeXT data directory.
No OSC or running SurgeXT required — pure filesystem write.

Local (run on target):
    python install_sonic_boom.py --user-patch-dir "/home/user/Documents/Surge XT/Patches"

Remote (run on host, copies via SCP):
    python install_sonic_boom.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip 192.168.1.100
    python install_sonic_boom.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip user@192.168.1.100
...
_XML = (
    ...
    '<patch revision="9">'
    '<meta name="SonicBoom" category="QNX" comment="" author="QNX" />'
    '<parameters>'
...
def _deploy_local(fxp_bytes: bytes, user_patch_dir: str) -> None:
    ...
    out_path = os.path.join(out_dir, "SonicBoom.fxp")
...
def _deploy_remote(fxp_bytes: bytes, user_patch_dir: str, target: str) -> None:
    ...
    remote_path = remote_dir + "/SonicBoom.fxp"
...
def main():
    parser = argparse.ArgumentParser(description="Install SonicBoom.fxp to SurgeXT Third Party Patches > QNX")
