"""
install_ese_normal.py
Writes ESE_Normal.fxp to patches_3rdparty/QNX/ in the SurgeXT data directory.
No OSC or running SurgeXT required — pure filesystem write.

Local (run on target):
    python install_ese_normal.py --user-patch-dir "/home/user/Documents/Surge XT/Patches"

Remote (run on host, copies via SCP):
    python install_ese_normal.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip 192.168.1.100
    python install_ese_normal.py --user-patch-dir "/home/user/Documents/Surge XT/Patches" --target-ip user@192.168.1.100

Restart SurgeXT after running to see the patch under Third Party Patches > QNX.
"""
...
_XML = (
    ...
    '<patch revision="9">'
    '<meta name="ESE_Normal" category="QNX" comment="" author="QNX" />'
    '<parameters>'
    ...
    '<a_filter1_cutoff type="2" value="5.000000" />'
    '<a_filter1_resonance type="2" value="0.200000" />'
    ...
    '<a_env1_decay type="2" value="-3.000000" />'
    ...
    '<a_volume type="2" value="0.900000" />'
...
def _deploy_local(fxp_bytes: bytes, user_patch_dir: str) -> None:
    out_dir = os.path.join(user_patch_dir, "QNX", "ESE")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "ESE_Normal.fxp")
...
def _deploy_remote(fxp_bytes: bytes, user_patch_dir: str, target: str) -> None:
    ...
    remote_path = remote_dir + "/ESE_Normal.fxp"
...
def main():
    parser = argparse.ArgumentParser(description="Install ESE_Normal.fxp to SurgeXT Third Party Patches > QNX")
    ...
    fxp = _build_fxp("ESE_Normal", _XML)
